...
class TestViewPage():
    ...
    def test_login_func(self):
        sf = SampleFile(self.driver)
        self.ut.enter_page("http://www.uitestingplayground.com/sampleapp")
        sf.check_login("", "pws")


    def test_login_func2(self):
        sf = SampleFile(self.driver)
        self.ut.enter_page("http://www.uitestingplayground.com/sampleapp")
        sf.check_login("test", "pwd")
    
    def test_mouse_clicks(self):
        ma = MouseAction(self.driver)
        self.ut.enter_page("http://www.uitestingplayground.com/mouseover")
        ma.check_mouse_click()
   

    def test_shadow_dom(self):
        sd = ShadowDom(self.driver)
        self.ut.enter_page("http://www.uitestingplayground.com/shadowdom")
        sd.compare_with_shadowdom()
        
     

    def test_alerts(self):
        ia = Alerts(self.driver)
        self.ut.enter_page("http://www.uitestingplayground.com/alerts")
        ia.interacting_with_alerts()
    
    def test_file_upload(self):
        fu = FileUpload(self.driver)
        self.ut.enter_page("http://www.uitestingplayground.com/upload")
        fu.upload_files()


    def test_animated_button(self):
        ab = AnimatedButton(self.driver)
        self.ut.enter_page("http://www.uitestingplayground.com/animation")
        ab.animated_button_handle()
        time.sleep(7)
